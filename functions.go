...
func (model *ServiceRegistryEntryInput) Save() *ServiceRegistryEntryInput {
	...
	stmt, err := db.Prepare("INSERT INTO Services (serviceDefinition,serviceDefinitionCreatedAt, serviceDefinitionUpdatedAt,  systemName, address, port, authenticationInfo, providerCreatedAt, providerUpdatedAt, serviceURI, endOfValidity, secure, version, createdAt, updatedAt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")

	currentTime := time.Now().Format("2006-01-02 15:04:05")

	handleError("could prepare statement: %v", err)
	fmt.Println("Registration recived", model.ProviderSystem.SystemName)
	res, err := stmt.Exec(model.ServiceDefinition, currentTime, currentTime, model.ProviderSystem.SystemName, model.ProviderSystem.Address, model.ProviderSystem.Port, model.ProviderSystem.AuthenticationInfo, currentTime, currentTime, model.ServiceUri, model.EndOfvalidity, model.Secure, model.Version, currentTime, currentTime)
...
func (model *Register) Find(name string) *Register {
	row := db.QueryRow("SELECT * FROM Services WHERE serviceDefinition = ?", name)
...
func (model *Register) Delete(name string) bool {
	stmt, err := db.Prepare("DELETE FROM Services WHERE serviceDefinition = ?")
